...
public class EditScheduleItemUpdateNavigator extends Navigator {

	@Override
	public boolean performTask(HttpServletRequest req, HttpServletResponse resp) {
		int isScheduleItem = Integer.parseInt(getParm(req, "idscheduleitem"));
		...
		String scheduleTimeString = getParm(req, "scheduleTimeText");

		ScheduleItemDTO scheduleItemDTO = new ScheduleItemDTO();
		scheduleItemDTO.setIdScheduleItem(isScheduleItem);
		...
		try {
			new DBScheduleItem(conn).updateItem(scheduleItemDTO);
		} catch (SQLException e) {
			setErrorMessage("Could not update schedule item");
			setErrorException(e);
			return false;
		} finally {
